 ...
 *
 * <p>Lives in a {@code .config} sub-package on purpose: the shared {@code
 * config/spotbugs-exclude.xml} scopes its {@code THROWS_METHOD_THROWS_CLAUSE_BASIC_EXCEPTION}
 * exemption to {@code .config.} packages, because {@code HttpSecurity.build()} forces every {@link
 * SecurityFilterChain} bean to declare {@code throws Exception}.
 *
 * <p>Before encryption this server only ever handed out {@code ${PLACEHOLDER}} strings, so leaving
 * it open cost nothing. It now returns real credentials, and {@code /decrypt} will turn any cipher
